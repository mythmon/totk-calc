...
export async function GET(_request: NextRequest, { params }: { params: { actorName: string } }): Promise<Response> {
  ...
  if (targetRow === null) return NextResponse.json({ error: "armor not found"}, { status: 404 });

  let imageInfo = armor.getImages().find(d => d.range.tl.nativeRow === targetRow! - 1)
  if (!imageInfo) return NextResponse.json({ error: "no image in row"}, { status: 404 });
